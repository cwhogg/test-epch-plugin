...
- **Any deck (payer / product-scoping / fundraise)** → also load `../skills/marley-visual-brand/colors.md`, `typography.md`, `logo-usage.md`, `image-style.md`, `layouts.md`, and `slide-patterns.md`. You are a visual thinker — pay attention to visual consistency, hierarchy, slide structure, brand fidelity, and **per-slide visual specificity**: does each slide name a specific layout from `layouts.md`, a specific composition pattern from `slide-patterns.md`, specific brand assets (icon SVG filenames, photo JPG filenames, named logo lockup), and a surface color? Or does the draft default to generic visual descriptions and one repeated layout/pattern/color?
...
- **Composition pattern named per slide?** Each content slide should name a pattern from `../skills/marley-visual-brand/slide-patterns.md` (e.g. `hero-stat-grid`, `2-column-pivot`, `horizontal-process-flow`, `n-box-equal-grid`, `radial-team-diagram`, `annotated-timeline`, `multi-chart-grid + stat-strip`, `positioning-diagram`, `3-column-option-compare`, `stat-stack`). The pattern carries composition rules (element count, asset role, surface tendencies) that the layout alone doesn't specify. Flag slides described only with a layout but no pattern.
- **Layout + pattern diversity across the deck?** The deck should use at least **4 distinct layout types** and **6+ distinct patterns** across ~25 slides. Flag decks where >70% of slides share one layout (e.g. every slide is `TITLE_AND_BODY` text blocks) or where the same pattern repeats more than ~3 times consecutively without variation.
...
- **Pattern-fit per slide's job?** When a pattern is named, does it fit the slide's communicative job per `slide-patterns.md`? E.g. `hero-stat-grid` only on the problem-framing slide at position 4 (never mid-deck); `stat-stack` only on engagement-KPI slides; `annotated-timeline` only on patient-case slides; `radial-team-diagram` only on care-team slides. Flag patterns used out of canonical position or with mismatched content.
- **Customer-specific patterns invoked appropriately?** Patterns like `geographic-map`, `gantt-chart`, `diagram-with-arrows (FFS→VBC)`, `logos-grid (payer portfolio)` should be used only when audience calls for them — health-system audiences (OSF-like), multi-state-payer audiences. Flag a `logos-grid` of competitor payers on a customer-specific deck (awkward); flag a `gantt-chart` on a consumer-facing artifact (wrong audience).
- **Dropped patterns not reached for?** Patterns like `checklist-vs-status-table`, `tabbed-card`, `9-icon services grid`, `distribution-bar-breakdown` ROI waterfall, `8-box n-grid`, `radial-ecosystem-diagram` were dropped in the Jan 2024 redesign. Flag any draft reaching for these — `slide-patterns.md` lists them under "Dropped patterns (don't use)."
...
- **Visual specificity (decks only):** pass / partial / fail — [one sentence on whether slides name specific layouts from `layouts.md`, specific composition patterns from `slide-patterns.md`, specific brand assets (icon SVG, photo JPG, logo lockup), and a surface color; or default to generic descriptions and uniform layout/pattern/palette]
```
